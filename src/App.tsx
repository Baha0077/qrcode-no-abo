...
function handleDownloadFramePNG(
  ...
  // Find the outer frame SVG and the inner QR SVG
  ...
  foreignObjects.forEach(fo => {
    if (innerQrSvg) {
      ...
      // Clone the inner QR SVG and strip the logo <image> elements
      ...
      // Set position and size via nested <svg> element
      ...
      // Copy QR SVG children into nested svg
      while (qrClone.firstChild) {
        nestedSvg.appendChild(qrClone.firstChild);
      }

      // Also carry over clipPath from foreignObject if any
...
function handleDownloadPNG(
  ...
  // Remove <image> elements - browsers won't render them when SVG is loaded as image src
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          ...
                          {logoSrc && (
                            <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">Logo-Gr&ouml;&szlig;e: {logoSize}%</label>
                              <input
                                type="range"
                                min="10"
                                max="35"
                              ...
                              {logoSize > 25 && (
                                <p className="text-xs text-amber-600 mt-1">&#9888; Gro&szlig;e Logos k&ouml;nnen die Scanbarkeit beeintr&auml;chtigen</p>
                              )}
                            </div>
                          )}
                        </div>
                      ...
                      {selectedFrame === 'none' ? (
                        <div className="space-y-2">
                          ...
                          <button
                            onClick={() => { incrementCounter(); handleDownloadSVG(qrRef, `${getPrefix()}.svg`, qrSizeCm, qrDpi); }}
                          ...
                          <button
                            onClick={() => { incrementCounter(); handleDownloadEPS(qrRef, `${getPrefix()}.eps`, qrSizeCm, qrColor, logoSrc); }}
                        ...
                        <div className="space-y-2">
                          ...
                          <button
                            onClick={() => { incrementCounter(); handleDownloadFrameSVG(qrRef, `${getPrefix()}-${selectedFrame}.svg`); }}
                            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors cursor-pointer font-medium"
                          >
                            <Download className="w-4 h-4" />
                            {t.svgMitRahmen}
